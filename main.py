...
from graph.graph import handle_new_data
...
def handle_graph(vertices: DataFrame, edges: DataFrame, epoch_id: int) -> None:
    ...
    """
    Callback appelé à chaque micro-batch avec les DataFrames raw des edges et vertices (pas encore agrégés).

    Parameters
    ----------
    vertices : DataFrame  — colonnes : id (str), type (USER | PROD | SEL)
    edges    : DataFrame  — colonnes : src (str), dst (str), relationship (str), price (float), last_ts_event (timestamp)
    epoch_id : int        — numéro du batch courant
    """
    edges = edges.withColumnRenamed("weight", "price")  #
